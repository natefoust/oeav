...
LPCSTR CColor::GetNameFromIndex(int i)
{
  ...
  return reinterpret_cast<LPCSTR>(m_namedColor[i].name);
}
...
{
  CColor t;
  t.SetString(reinterpret_cast<LPCTSTR>(pcColor));
  return t;
}
...
{
  CString color;
  color.Format(reinterpret_cast<const wchar_t>("%02X%02X%02X"), GetRed(), GetGreen(), GetBlue());
  return color;
}
...
bool CColor::SetString(LPCTSTR pcColor) 
{
  ASSERT(pcColor);
  int r, g, b;
  if (sscanf_s(reinterpret_cast<const char *>(pcColor), "%2x%2x%2x", &r, &g, &b) != 3) 
  {
